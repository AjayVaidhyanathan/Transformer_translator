...
def translate(sentence: str):
    ...
    if type(sentence) == int or sentence.isdigit():
        id = int(sentence)
        ds = load_dataset('opus_books', f"{config['lang_src']}-{config['lang_tgt']}", split='all')
    ...
    with torch.no_grad():
        ...
        print("Source Pre-Tokenized:", source)
        source = torch.cat([
    ...
    # Calculate the decoder output
            ...
            # Project to token probabilities and pick the next word
...
translate(sys.argv[1] if len(sys.argv) > 1 else "Guten Tag")